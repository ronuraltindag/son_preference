...
************************************************************************
************************************************************************
log using "$root/3analysis/do files/final revision/logfiles/Table 92.log", replace  
...
keep if hw5!=. & hw5<9000

g stunt=(hw5<-200) 
g underw=(hw8<-200)

***Outcome means and sds in Table 9, Panels (2) and (3)***
...
eststo clear 
eststo:reg stunt prev_f##female  if bord>2, cluster(mid) 
lincom 1.female + 1.prev_f#1.female
estadd scalar inte=r(estimate) 
estadd scalar intse=r(se) 

eststo:reg stunt prev_f##female i.bord i.wave##i.region $regset1 $regset3 m_*  if bord>2, cluster(mid)
lincom 1.female + 1.prev_f#1.female
estadd scalar inte=r(estimate) 
estadd scalar intse=r(se) 

eststo:reg underw prev_f##female  if bord>2, cluster(mid)
lincom 1.female + 1.prev_f#1.female
estadd scalar inte=r(estimate) 
estadd scalar intse=r(se) 

eststo:reg underw prev_f##female i.bord i.wave##i.region $regset1 $regset3 m_*  if bord>2, cluster(mid)
lincom 1.female + 1.prev_f#1.female
estadd scalar inte=r(estimate) 
estadd scalar intse=r(se) 
***Diff.-in-diff. estimaete in Table8, Panels (2) and (3)***
...
esttab  using "Table 92.csv", replace se r2  ///
