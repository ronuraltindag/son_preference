...
************************************************************************
************************************************************************
log using "$root/3analysis/do files/final revision/logfiles/Appendix Table 4p2", replace 

global regeduc1 "MotherAge hage firstbirthage"
global regeduc2 "nonturkish region rural extfamily arranged dowry heducat wave"
global regeduc3 "i.heducat i.nonturkish i.region i.rural i.extfamily i.arranged i.dowry i.wave" 
...
keep if MotherAge>34 // Older sample of women 


foreach i in $regeduc1{
g m_`i'=`i'==. 
qui sum `i' 
...
foreach i in $regeduc2{
recode `i' (.=999)
}

eststo: reg nbliving girl1##educat ChildAlive $regeduc1 $regeduc3 m_* , robust
lincom 1.girl1+1.girl1#0.educat
estadd scalar coef1=r(estimate) 
estadd scalar se1=r(se)
lincom 1.girl1+1.girl1#1.educat
estadd scalar coef2=r(estimate) 
estadd scalar se2=r(se) 
lincom 1.girl1+1.girl1#2.educat 
estadd scalar coef3=r(estimate) 
estadd scalar se3=r(se)
qui sum nbliving if educat==0 & e(sample)==1 & girl1==0 
estadd scalar mean1=r(mean) 
qui sum nbliving if educat==1 & e(sample)==1 & girl1==0 
estadd scalar mean2=r(mean) 
qui sum nbliving if educat==2 & e(sample)==1 & girl1==0 
estadd scalar mean3=r(mean) 

testparm 1.girl1#1.educat 1.girl1#2.educat


eststo: poisson nbliving girl1##educat ChildAlive $regeduc1 $regeduc3 m_* , robust
lincom 1.girl1+1.girl1#0.educat 
estadd scalar coef1=r(estimate) 
estadd scalar se1=r(se)
lincom 1.girl1+1.girl1#1.educat
...
esttab  using "Online Table 4.2 - educat.csv", replace se r2 pr2 ///
